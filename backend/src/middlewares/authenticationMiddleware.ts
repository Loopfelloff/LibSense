...
const jwtVerify = promisify(jwt.verify) as (accessToken : string , refreshToken :string)=> Promise<jwt.JwtPayload> 
...
const authenticationMiddleware = async (req : Request , res : Response,  next : NextFunction)=>{
	...
	if(err instanceof Error){
		...
		try {
		    ...
		    const newRefreshToken =  jwt.sign(payload , String(process.env.REFRESH_TOKEN_SECRET) , { expiresIn : '30d' })
