...
export const getAuthorDetail = async (req: Request<{ author_id: string }>, res: Response) => {
  try {
    ...
    const author = await prisma.bookAuthor.findUnique({  // Changed from bookAuthor to book_author
      where: { id: author_id },
      select: {
        id: true,
        ...
        book_written_by: {  // Changed from book_written_by to BookWrittenBy
          select: {
            book: {
              select: {
                id: true,
    ...
    const books = author.book_written_by.map((bw) => bw.book)  // Changed from book_written_by to BookWrittenBy
