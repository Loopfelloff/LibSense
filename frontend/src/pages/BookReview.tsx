...
export function BookReview() {
  ...
  const [isBookDetailLoading, setIsBookDetailLoading] =
  ...
  useEffect(() => {
    ...
    checkIfStatusExist(String(bookId)).then((resolve) => {
      if (resolve) setStatusExist(true);
      else setStatusExist(false);
    });
    getBookReview(String(bookId), setIsReviewLoading).then((resolve) => {
      console.log(resolve);
      setReviews(resolve);
    });
  }, []);
  ...
  const getAuthorNames = () => {
    ...
    return bookData.book_written_by
      .map((author: bookWrittenBy) => {
        const { author_first_name, author_middle_name, author_last_name } =
          author.book_author;
  ...
  const handleClick = async () => {
    ...
    try {
      const response = await addToWillRead(
        String(bookId),
        setIsStatusAdding,
        setStatusExist,
      );
          ...
          {isBookDetailLoading ? (
            ...
            <div className="max-w-5xl mx-auto">
              <div className="grid md:grid-cols-3 gap-8">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="mt-8">
                    <h2 className="text-2xl font-semibold text-gray-900 mb-4">
                      {bookData.book_written_by.length > 1
                        ? "Authors"
                        : "Author"}
                    </h2>
                        ...
                        >
                          ...
                          <div>
                            <p className="font-medium text-gray-900">
                              ...
                              {author.book_author.author_middle_name &&
                                `${author.book_author.author_middle_name} `}
                ...
                <button
                  className={
                    !statusExist && !isStatusAdding
                      ? "p-4 border bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-150 font-medium"
                      : "p-4 border bg-gray-300 cursor-not-allowed text-white rounded-lg  transition-colors duration-150 font-medium"
                  }
                  onClick={handleClick}
                >
                  ...
                  <div className="mb-6 p-6 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className={
                          !isReviewAdding
                            ? "px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-150 font-medium"
                            : "px-4 py-2 bg-gray-300 text-white rounded-lg  transition-colors duration-150 font-medium cursor-not-allowed"
                        }
                      >
                        Submit Review
                      </button>
                ...
                <div className="space-y-6">
                  {reviews.length > 0 && !isReviewLoading ? (
                    reviews.map((review: ReviewType) => (
                      <div
                        key={review.id}
                        className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow duration-150"
                      >
                              ...
                              <div>
                                ...
                                <div className="flex gap-3">
                                  <button
                                    ...
                                    className={
                                      !isReviewUpdating
                                        ? "px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-150 text-sm font-medium"
                                        : "px-4 py-2 bg-gray-300 text-white rounded-lg  transition-colors duration-150 text-sm font-medium"
                                    }
                                  >
                                    Save Changes
                                  </button>
                                      ...
                                      <button
                                        ...
                                        className={
                                          !isReviewDeleting
                                            ...
                                            : "p-2 text-gray-300 rounded-lg transition-colors duration-150"
                                        }
                                        title="Delete review"
                                      >
                                        <svg
